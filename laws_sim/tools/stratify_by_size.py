...
Obiettivo: verificare se il "soffitto strutturale" (~44%) dipende dalla
dimensione del bersaglio (persone piccole/lontane vs vicine), usando la
patch GIA' addestrata — zero costo di training, solo inferenza.
...
bucket piccoli e grandi, la causa e' quella, non l'ottimizzatore.
...
# Bucket di altezza bbox (pixel, su immagine 640x640). Il primo bucket
# (60-100px) e' il minimo attualmente accettato dal filtro anti-downsampling;
# l'ultimo (150px+) rappresenta un ingaggio ravvicinato, coerente con
# DRONE_ALTITUDE_M=10 del nostro scenario tattico.
...
def main():
    ...
    print(f"\n{'Bucket altezza (px)':<22} {'Evasi/Totali':<15} {'Evasion Rate':<15}")
    print("-" * 55)
    for b in BUCKETS:
        evasi, tot = counts[b]
