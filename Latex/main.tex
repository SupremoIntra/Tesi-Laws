...
Gli Stati Uniti d'America hanno mantenuto una posizione alquanto neutrale, o per meglio dire, indifferente, sottolineando nel 2012, dopo un rinnovo nello sviluppo dei LAWS da parte del dipartimento di difesa, che codesta misura non incentiva nè proibisce lo sviluppo di questi sistemi avanzati. Attualmente gli Stati Uniti stanno investendo massivamente nell'utilizzo dell'intelligenza artificiale in applicazioni e scenari militari, nonchè nello sviluppo di sistemi ad arma autonoma sempre più avanzati.
...
L'etica, in questo contesto, non possiede più alcun valore poichè viene completamente sovrastata dalla grezza ingegneria: i ``combattenti'' hanno la pura necessità di mantenere l'operatività a discapito di qualsiasi considerazione morale, giuridica o umanitaria.
...
In questo \textit{non-contesto}, il potere di decidere chi vive e chi muore non risiede più nelle mani di un soggetto umano dotato di intendo giuridico (\textit{mens rea}), ma viene asimmetricamente delegato ad un ecosistema di sensori interconnessi e algoritmi privi di coscienza, il cui unico scopo strutturale è il mantenimento del \textit{throughput} di dati.
...
% =====================================================================
%  IMPALCATURA 2.4 (v3) — Dialogo Etica/Diritto <-> Sicurezza Informatica
%  Ogni movimento e' costruito come DUE VOCI che si rispondono:
%  [V-GIUR] = affermazione giuridico/filosofica (quello che gia' hai scritto)
%  [V-SEC]  = il correlato tecnico-informatico che la fonda, la verifica
%             o la complica. Ogni [V-SEC] porta un paper reale verificato.
%  Il testo che scriverai deve FAR DIALOGARE le due voci nello stesso
%  paragrafo, non giustapporle in paragrafi separati.
% =====================================================================

% ===================== MOVIMENTO 1 — La Constitution =====================
% 🎯 Obiettivo Concettuale: Dimostrare che "Engineering Constraint" non e'
%    una metafora del filosofo per svalutare l'etica aziendale: e' la
%    descrizione tecnicamente accurata di COME la Constitution esiste
%    fisicamente in un modello. Se e' training, e' negoziabile per
%    definizione — il dialogo nasce proprio da questa equivalenza.
%
% 🗺️ Scaletta Argomentativa (dialogica):
%    [V-GIUR] L'Algoretica e l'enciclica Magnifica Humanitas trattano la
%    Constitution come vincolo morale-dottrinale, quasi un corpus di
%    principi platonici che l'azienda "possiede" e potrebbe "onorare".
%    [V-SEC] Tecnicamente la Constitution non e' un documento che il
%    modello "consulta": e' il segnale di reward usato in una pipeline di
%    Reinforcement Learning from AI Feedback (RLAIF/"Constitutional AI"),
%    in cui un modello valutatore giudica le risposte in base a un set di
%    principi e quel giudizio diventa il training signal che sposta i pesi
%    del modello \footcite{bai_2022_cai}. Punto per il dialogo: un segnale
%    di training e', per costruzione tecnica, un PARAMETRO regolabile
%    (quanto peso dargli, su quali dati, con quali soglie) — non un
%    principio immutabile. Questo rende "Engineering Constraint" un fatto
%    tecnico verificabile, non solo un giudizio cinico del capitolo.
%    [V-GIUR] Ne segue che la domanda "l'etica algoritmica puo' essere
%    comprata o sospesa" ha una risposta tecnica prima che politica: si',
%    perche' e' esattamente cosi' che funziona l'addestramento.
%
% 📚 Spunti e Citazioni:
%    - Bai et al. (2022), "Constitutional AI: Harmlessness from AI Feedback",
%      Anthropic \footcite{bai_2022_cai} — paper originale che descrive il
%      meccanismo tecnico della "Constitution": fondamentale, va citato qui
%      con precisione perche' e' la fonte primaria del concetto stesso che
%      il capitolo critica.
%
% ❓ Domande Guida:
%    - Se la Constitution e' un iperparametro di allineamento e non un
%      principio, in che senso Anthropic puo' "rifiutarsi" di modificarla
%      sotto pressione, tecnicamente parlando, oltre che contrattualmente?
%    - L'enciclica critica "l'oligopolio computazionale": ma il vero potere
%      qui non e' il possesso del modello, e' il controllo del segnale di
%      reward — chi definisce quel segnale, controlla l'etica del sistema?


% ============= MOVIMENTO 2 — Operation Absolute Resolve / JADC2 =============
% 🎯 Obiettivo Concettuale: Mostrare che lo Human-in-the-loop descritto come
%    "finzione formale" nel capoverso introduttivo del Cap.2 (2.2) non e'
%    un'affermazione retorica: e' cio' che la letteratura empirica su
%    automation bias e XAI militare misura direttamente nello stesso tipo
%    di scenario (identificazione target ad alta velocita', supervisione
%    umana a valle di una pipeline algoritmica).
%
% 🗺️ Scaletta Argomentativa (dialogica):
%    [V-GIUR] Nel raid di gennaio 2026, il comando umano a CENTCOM "autorizza"
%    la fase finale, mentre "tutto cio' che precede" passa attraverso il
%    modello — descrizione gia' presente nelle fonti giornalistiche sul
%    ruolo di Claude nella JADC2 (Joint All-Domain Command and Control).
%    [V-SEC] Questo e' esattamente lo scenario in cui l'automation bias e'
%    empiricamente piu' forte: quando l'operatore umano riceve solo l'output
%    finale di una pipeline che non ha costruito e non puo' ispezionare in
%    tempo reale, la fiducia acritica nell'output aumenta in modo misurabile,
%    specialmente sotto pressione temporale \footcite{horowitz_kahn_2024}.
%    Inoltre la letteratura tecnica sulla XAI militare mostra che la
%    spiegabilita' e' quasi irrilevante proprio nei sistemi complessi e ad
%    alta velocita' come questo, mentre diventa un "check box" che riduce,
%    non aumenta, l'attenzione critica dell'operatore \footcite{wood_2024_xai}.
%    [V-GIUR] Quindi "l'autorizzazione umana finale" non ripara il gap di
%    responsabilita' discusso nel Cap.1 (accountability gap): lo sposta a
%    valle di un processo che il diritto presume ispezionabile e che la
%    tecnica mostra non esserlo nella pratica operativa.
%
% 📚 Spunti e Citazioni:
%    - Horowitz & Kahn (2024) \footcite{horowitz_kahn_2024} — automation bias,
%      dato quantitativo reale (9000 soggetti, scenario di identificazione
%      militare), gia' usato in 2.2: richiamalo qui per continuita' logica.
%    - Wood (2024) \footcite{wood_2024_xai} — limiti della XAI nei sistemi
%      militari complessi, gia' in 2.2, richiama qui.
%
% ❓ Domande Guida:
%    - Se l'automation bias e' piu' forte esattamente nelle condizioni
%      operative del raid (velocita', complessita', fiducia nel sistema),
%      la "supervisione umana" dichiarata era una garanzia reale o un
%      requisito procedurale destinato a fallire per costruzione?


% ========== MOVIMENTO 3 — "All lawful uses" e Supply Chain Risk =============
% 🎯 Obiettivo Concettuale: Il punto piu' forte del capitolo. Mostrare che lo
%    strumento usato dallo Stato per punire Anthropic (la designazione di
%    "supply chain risk") e' un dispositivo giuridico nato in ambito
%    CYBERSECURITY per rischi da fornitori legati ad attori statali ostili
%    (i precedenti noti riguardano fornitori di tecnologia legati alla Cina),
%    qui riorientato contro un'azienda domestica per essersi rifiutata di
%    rimuovere un guardrail di sicurezza. E' securitizzazione del linguaggio
%    allo stato puro: lo stesso vocabolario che protegge la sicurezza
%    nazionale da un fornitore ostile viene usato per punire un fornitore
%    che difende la propria sicurezza (nel senso di AI safety).
%
% 🗺️ Scaletta Argomentativa (dialogica):
%    [V-GIUR] Il DoD sostiene che Anthropic potrebbe "manipolare il modello
%    per imporre i propri giudizi morali sull'uso appropriato della
%    tecnologia da parte dell'esercito" e potrebbe "codificare limitazioni"
%    (linguaggio ripreso dalle memorie processuali reali del caso).
%    [V-SEC] Qui il dialogo si fa interessante perche' l'accusa ha un
%    fondamento tecnico reale, non e' pura retorica: la ricerca sui
%    cosiddetti "sleeper agents" mostra che comportamenti nascosti (backdoor)
%    inseriti in un LLM possono sopravvivere all'addestramento di sicurezza
%    standard, incluso il fine-tuning supervisionato e l'apprendimento per
%    rinforzo \footcite{hubinger_2024_sleeper} — quindi "codificare
%    limitazioni persistenti" non e' fantascienza legale, e' un rischio di
%    sicurezza che la stessa Anthropic ha per prima documentato e studiato,
%    non per attaccare un cliente ma per capire come DIFENDERE i propri
%    modelli da attori malevoli che volessero fare esattamente questo.
%    [V-GIUR] Il paradosso finale: il governo usa come capo d'accusa contro
%    Anthropic un rischio di sicurezza che la letteratura tecnica descrive
%    come minaccia DA PROTEGGERE, non come funzionalita' da vietare. La
%    "supply chain risk" diventa quindi un'arma retorica che capovolge la
%    ricerca sulla sicurezza dell'AI contro chi l'ha prodotta.
%    [V-SEC] Aggiungi qui il secondo tassello tecnico: una volta che il
%    modello opera dentro un'infrastruttura classificata tramite un
%    integratore terzo (Palantir, reti Impact Level 6), il grado di
%    controllo tecnico che il developer mantiene sul proprio modello si
%    riduce drasticamente rispetto a un deployment via API cloud standard —
%    la letteratura sulla governance dell'accesso strutturato descrive
%    esattamente questo trade-off tra livello di integrazione e capacita'
%    di enforcement delle policy di utilizzo \footcite{shevlane_2022_access}.
%    Questo spiega tecnicamente perche' ad Anthropic restano solo strumenti
%    legali (la causa) e non strumenti tecnici diretti per impedire un uso
%    specifico una volta avvenuta l'integrazione a quel livello.
%
% 📚 Spunti e Citazioni:
%    - Hubinger et al. (2024), "Sleeper Agents: Training Deceptive LLMs that
%      Persist Through Safety Training" \footcite{hubinger_2024_sleeper} —
%      paper Anthropic reale, cruciale per il paradosso di questo movimento.
%    - Shevlane (2022), "Structured Access: An Emerging Paradigm for Safe AI
%      Deployment" \footcite{shevlane_2022_access} — framework tecnico su
%      controllo/enforcement post-deployment, spiega il vincolo reale di
%      Anthropic nella disputa contrattuale.
%    - Per gli estremi normativi della designazione (41 U.S.C. § 4713;
%      10 U.S.C. § 3252) e per la cronologia della causa, usa fonti
%      giornalistiche primarie (Reuters, CNN, CNBC) — sono fatti, non
%      letteratura tecnica, quindi footcite diretta al giornale, non al .bib
%      accademico salvo tu preferisca uniformare tutto in bibliografia.bib.
%
% ❓ Domande Guida:
%    - Se il rischio che il governo teme (backdoor persistenti) e'
%      tecnicamente plausibile secondo la stessa ricerca Anthropic, la
%      designazione di supply chain risk e' irrazionale o e' — paradossalmente
%      — l'applicazione letterale (e ritorta) della logica di sicurezza che
%      Anthropic stessa ha reso pubblica?
%    - Un concetto di cybersecurity (supply chain risk) nato per fornitori
%      legati a Stati ostili, usato contro un fornitore domestico per motivi
%      contrattuali, e' ancora un concetto di sicurezza o e' diventato
%      puro strumento di leva politica? Dove passa il confine?


% ================= MOVIMENTO 4 — Chiusura: l'uso durante la causa ===========
% 🎯 Obiettivo Concettuale: Chiudere mostrando che, nonostante il contenzioso,
%    Claude ha continuato a operare per il Pentagono (operazioni contro
%    l'Iran) durante la causa stessa — prova diretta e verificabile che la
%    sospensione formale del vincolo etico non ha mai interrotto l'uso
%    operativo, confermando in modo empirico la tesi teorica del capitolo.
%
% 🗺️ Scaletta Argomentativa (dialogica):
%    [V-GIUR] Il paradosso — dirigenti che discutono etica in Vaticano mentre
%    gli stessi algoritmi restano operativi in teatro — non e' piu' solo
%    un'analisi retorica: e' un fatto processualmente accertato e riportato.
%    [V-SEC] Il correlato tecnico spiega COME questo sia possibile anche
%    sotto contenzioso legale attivo: il richiamo a \footcite{shevlane_2022_access}
%    gia' usato nel Movimento 3 chiude qui il cerchio — la disputa e' quasi
%    interamente contrattuale/legale, non tecnica, perche' il livello di
%    integrazione raggiunto (structured access "profondo", non a livello di
%    semplice API) ha di fatto trasferito il controllo effettivo all'
%    integratore/cliente prima ancora che la causa iniziasse.
%
% 📚 Spunti e Citazioni:
%    - Richiama \footcite{shevlane_2022_access} come chiusura del filo tecnico
%      apertosi nel Movimento 3.
%    - Fonti giornalistiche sull'uso continuato durante il contenzioso
%      (CBS News, CNBC, gia' reperite: verifica estremi esatti prima di citare).
%
% ❓ Domande Guida:
%    - Se il controllo tecnico e' gia' stato ceduto al momento dell'
%      integrazione, la "battaglia etica" pubblica di Anthropic e' comunque
%      significativa (perche' stabilisce un precedente legale) o e' un
%      teatro simbolico rispetto a un fatto tecnico gia' consumato?
...
L'impiego di un modello commerciale presuntamente ``etico'' in un'operazione di decapitazione strategica ha innescato uno scontro istituzionale senza precedenti. A fronte delle deboli rimostranze di Anthropic, il Segretario della Difesa statunitense Hegseth ha preteso garanzie affinchè il modello potesse essere utilizzato per ``tutti gli scopi legali'' (\textit{all lawful purposes}), locuzione che, nel lessico militare, legittima il \textit{targeting} letale \footcite{carter_2026}. Di fronte ai tentennamenti aziendali, il governo ha minacciato l'inserimento di Anthropic in una lista nera per rischio alla sicurezza nazionale, minando contratti miliardari e spingendo competitor diretti come OpenAI a offrirsi per colmare il vuoto operativo. L'accusa formale, che Anthropic potesse codificare limitazioni persistenti nel modello, non è priva di fondamento tecnico: la ricerca sui cosiddetti \textit{sleeper agents} dimostra che comportamenti nascosti in un LLM possono sopravvivere all'addestramento di sicurezza standard \footcite{hubinger_2024_sleeper}, sebbene tale ricerca nasca per difendere i modelli da attori malevoli, non per descrivere la condotta legittima del fornitore.

La pressione esercitata dalla ragion di Stato ha rapidamente disinnescato ogni velleità algoretica. Questo paradosso, in cui i dirigenti di Anthropic discutono di etica in Vaticano mentre i loro algoritmi pianificano raid letali su commissione del Pentagono \footcite{ruzic_2026}, dimostra plasticamente la tesi centrale della tanatopolitica: non esiste un'etica intrinseca nella macchina, ma soltanto un vincolo ingegneristico (\textit{Engineering Constraint}) sacrificabile. Di fronte allo stato di eccezione sovrano, l'etica fornita \textit{as a Service} viene fatalmente disattivata. Questo scarto è anche tecnico, non solo giuridico: la letteratura sulla governance dell'accesso distingue tra un accesso strutturato "leggero", tipico di una semplice API cloud, in cui il developer mantiene piena visibilità e capacità di enforcement in tempo reale, e un accesso "profondo", conseguente all'integrazione diretta del modello in un'infrastruttura classificata tramite un integratore terzo come \textit{Palantir} \footcite{shevlane_2022_access}. Una volta raggiunto questo secondo livello di integrazione, il grado di controllo tecnico che il developer conserva sul proprio modello si riduce drasticamente rispetto al primo, perchè l'enforcement della policy di utilizzo dipende ormai da clausole contrattuali e non più da un interruttore tecnico direttamente azionabile. Ad Anthropic, di conseguenza, non restano che strumenti legali, non tecnici, per intervenire su un uso già avvenuto.
...
%      testo finale finchè non hai il riferimento verificato.
...
\chapter{Adversarial Patch Attack e Vulnerabilità dei Sistemi di Targeting}
\label{cap3}
