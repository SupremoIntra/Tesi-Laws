...
Il punto tecnicamente rilevante non è che il sistema ignori gli effetti collaterali, ma che il suo ottimizzatore non li penalizzi esplicitamente. In termini di \textit{machine learning}, ciò che non compare nella \textit{loss function} non viene minimizzato: un modello addestrato a massimizzare il numero di target confermati, senza un termine di penalizzazione per il danno a non combattenti sufficientemente adeguato, genererà un \textit{bias} operativo, mirato all'efficienza. Non si tratta quindi di un errore contingente del sistema, ma diretta conseguenza della modalità con la quale è stato progettato, un caso di \textit{reward misspecification} applicato al dominio bellico. Analizzando questo complesso scenario, verrebbe da chiedersi se effetivamente possiamo parlare di un \textit{bias} effettivo o se, piuttosto, risulterebbe un ``addolcimento'', riscoprendosi in realtá un'ottimizzazione cinica e appositamente progettata per risultare cieca a quell'aspetto etico e non produttivo che è la vita umana.
